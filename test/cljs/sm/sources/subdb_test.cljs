...
(test "encode subdb hash"
  (let [hash (<? (subdb/subdb-hash "test/fixtures/sample1.file"))]
...
(test "search subdb for valid hash"
  (binding [subdb/*subdb-endpoint* helper/subdb-sandbox]
    (let [response (<? (subdb/subdb-search-languages helper/subdb-test-hash))]
...
(test "upload subtitle"
  (binding [subdb/*subdb-endpoint* helper/subdb-sandbox]
    (let [stream (node/create-read-stream "test/fixtures/subdb-download.srt")
          response (<? (subdb/subdb-upload helper/subdb-test-hash stream))]
...
(test "subdb expand results"
  (let [result {:language "pt" :count 2}
    ...
    (assert (= (subdb/subdb-expand-result "123" result) expected))))

(test "subdb integration stack"
  (with-redefs [subdb/subdb-hash (fn [path]
                                (assert (= path "sample-path"))
                                (go helper/subdb-test-hash))
                subdb/subdb-search-languages (fn [hash]
                                            (assert (= hash helper/subdb-test-hash))
                                            (go [{:language "es" :count 2}
                                                 {:language "pt" :count 1}]))
                subdb/subdb-download-stream (fn [hash lang version]
                                           (assert (= hash helper/subdb-test-hash))
                                           (assert (= lang "pt"))
                                           (assert (= version 0))
                                           "content")]
    (let [host (subdb/subdb-source)
